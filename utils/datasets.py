...
class DataManager(object):
    ...
    def __init__(self, dataset_name='imdb', dataset_path=None, image_size=(48, 48)):
        ...
        if self.dataset_path is not None:
            ...
            raise ValueError('Incorrect dataset name, please input imdb, fer2013, or KDEF')
    ...
    def _load_imdb(self):
        ...
        image_names = [image_name[0] for image_name in image_names_array]
    ...
    def _load_fer2013(self):
        ...
        for pixel_sequence in pixels:
            face = np.array([int(pixel) for pixel in pixel_sequence.split(' ')]).reshape(width, height)
    ...
    def _load_KDEF(self):
        ...
        file_paths = [os.path.join(folder, filename)
                      for folder, _, filenames in os.walk(self.dataset_path)
                      for filename in filenames if filename.lower().endswith('.jpg')]

        num_faces = len(file_paths)
        y_size, x_size = self.image_size
        faces = np.zeros((num_faces, y_size, x_size))
        emotions = np.zeros((num_faces, num_classes))
